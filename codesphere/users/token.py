...
Link: https://github.com/okuzmenko31/drf-ecommerce/blob/main/ecommerce/users/utils.py
"""
...
class TokenMixin(MailContextMixin):
    """
    ...
    """
    token_type: Optional[str] = None
    html_message_template: Optional[str] = None
    ...
    def _create_token(self, email: str) -> TokenData:
        ...
        """
        if not Token.objects.filter(owner_email=email,
                                    token_type=self.token_type).exists():
            token = Token.objects.create(owner_email=email,
                                         token_type=self.token_type)
        else:
            try:
                # If a token already exists, it will be deleted
                # and a new one will be created.
                Token.objects.get(owner_email=email,
